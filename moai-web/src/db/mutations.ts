...
export async function updateChat(
  chatId: string,
  userId: string,
  updates: { title?: string; visibility?: "user" | "organization" }
) {
  // Verify ownership
...
// Organization-scoped model operations
export async function createOrganizationModel(
  organizationId: string,
  createdByUserId: string,
  modelData: Omit<s.ModelInserts, "userId" | "organizationId"> & {
    name: string;
  }
) {
...
export async function updateOrganizationModel(
  modelId: string,
  organizationId: string,
  updates: Partial<Omit<s.ModelInserts, "userId" | "organizationId">>
) {
  // Verify organization ownership before update
  const model = await db.query.model.findFirst({
    where: (models, { and, eq }) =>
      and(eq(models.id, modelId), eq(models.organizationId, organizationId)),
  });
...
export async function duplicateModel(
  ...
  const modelData: s.ModelInserts = {
    ...originalModel,
    ...newModelData,
    version: 1, // Reset version for duplicated model
  };
...
export async function verifyModelOrganization(
  modelId: string,
  organizationId: string
) {
  const model = await db.query.model.findFirst({
...
export async function duplicateUserModel(
  ...
  return await duplicateModel(originalModelId, { ...newModelData, userId });
}

export async function duplicateOrganizationModel(
  originalModelId: string,
  organizationId: string,
  newModelData: Partial<Omit<s.ModelInserts, "organizationId">>
) {
  // Verify the model belongs to the organization
  await verifyModelOrganization(originalModelId, organizationId);

  return await duplicateModel(originalModelId, {
    ...newModelData,
    organizationId,
  });
}
