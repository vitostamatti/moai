...
import { VariableEditorSection } from "@/components/pages/variables-editor-page";
...
const Page = async ({ params }: PageProps) => {
  const { modelId, varId } = await params;
  ...
  const breadcrumbs = [
    ...
    {
      url: `/models/${model.model.id}/variables/${varId}`,
      label: "Editor",
    },
  ];
  ...
  return (
    <ModelShellClient modelId={modelId} breadcrumbs={breadcrumbs}>
      <VariableEditorSection modelId={modelId} varId={varId} />
    </ModelShellClient>
  );
};
